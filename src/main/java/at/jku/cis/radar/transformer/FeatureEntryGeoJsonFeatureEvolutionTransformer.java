    ...
    private List<GeoJsonFeatureEvolution> buildGeoJsonFeatureEvolutions(long featureGroup, Date date,
        ...
        if (!isGeometryEmpty(createdGeometryCollection)) {
            geoJsonFeatureEvolutions
                    .add(buildCreatedGeoJsonFeatureEvolution(featureGroup, date, createdGeometryCollection));
        }
        if (!isGeometryEmpty(erasedGeometryCollection)) {
            geoJsonFeatureEvolutions
                    .add(buildErasedGeoJsonFeatureEvolution(featureGroup, date, erasedGeometryCollection));
        }
        return geoJsonFeatureEvolutions;
    }

    private GeoJsonFeatureEvolution buildErasedGeoJsonFeatureEvolution(long featureGroup, Date date,
            Geometry geometry) {
        return buildGeoJsonFeatureEvolution(featureGroup, date, geometry, GeoJsonStatus.ERASED);
    }

    private GeoJsonFeatureEvolution buildCreatedGeoJsonFeatureEvolution(long featureGroup, Date date,
            Geometry geometry) {
        return buildGeoJsonFeatureEvolution(featureGroup, date, geometry, GeoJsonStatus.CREATED);
    }
